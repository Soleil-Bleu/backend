...
async def calculate_simulation_task(request: SimulationRequest, file_path: str):
    try:
        insert_data = {
            "form_id": request.id,
            ...
            "taux_pret": request.taux_pret,
            "duree_pret": request.duree_pret,
            "file_path": str(request.id),
            "status": "Received",
            "results": {}
        }
        supabase.table("simulations").insert(insert_data).execute()
        logger.info(f"Data inserted into database for request ID {request.id}")

        with open(file_path, "rb") as f:
            res = supabase.storage.from_("Enedis").upload(
                file=f, 
                path=str(request.id), 
        ...
        for puissance in puissances:
            args_simu.append([
                ...
                constantes_ENEDIS, 
                request.prix_achat, 
                request.type_centrale, 
                request.montant_pret, 
                request.taux_pret, 
                request.duree_pret
                ])

        points_simu = list(simulation.starmap(args_simu))
...
async def calc_simulation(
    ...
    import asyncio
    asyncio.create_task(calculate_simulation_task(simulation_request, file_location))
